...
const string strMessageMagic = "NavajoAnonBeta Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // navajoanonbeta: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // navajoanonbeta: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	if(pindexBest->nHeight < 1)
    {
    	...
    	else if(pindexBest->nHeight >= 14000 && pindexBest->nHeight < 25000)
    {
		nSubsidy = 150 * COIN;
    }
		else if(pindexBest->nHeight >= 25001 && pindexBest->nHeight < 35000)
    {
		nSubsidy = 15 * COIN;
    }
		else if(pindexBest->nHeight >= 35001 && pindexBest->nHeight < 50000)
    {
		nSubsidy = 1.5 * COIN;
    }
...
// navajoanonbeta: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes NavajoAnonBeta fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // navajoanonbeta: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // navajoanonbeta: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // navajoanonbeta: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // navajoanonbeta: track money supply and mint amount info
...
// navajoanonbeta: total coin age spent in transaction, in the unit of coin-days.
...
// navajoanonbeta: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // navajoanonbeta: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // navajoanonbeta: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // navajoanonbeta: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // navajoanonbeta: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 25; //coinbase maturity change to 35 blocks
    // Size limits
...
bool CBlock::AcceptBlock()
{
    ...
    // navajoanonbeta: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // navajoanonbeta: check proof-of-stake
    ...
    // navajoanonbeta: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // navajoanonbeta: getblocks may not obtain the ancestor block rejected
    ...
    // navajoanonbeta: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "NavajoAnonBeta", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Navajo Anon Beta To The Moon!";
        CTransaction txNew;
        txNew.nTime = 1408812954;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0 << CBigNum(42) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].SetEmpty();
        txNew.strTxComment = "text:NavajoCoin Anon Beta genesis block";
		CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1408812954;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("98c30dd2cec7e3ff2a528b24bf2aad44d518231ac7c61631d2bd150fb3591edf"));
        block.print();
        ...
        // navajoanonbeta: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x70, 0x40, 0x24, 0x10 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // navajoanonbeta: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // navajoanonbeta: tell downloading node about the latest block if it's
